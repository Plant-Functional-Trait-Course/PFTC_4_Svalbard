...
# Special case
specialFile <- read_xlsx(path = "raw_data/climate/DATA_ITEX_2015_2018/Hobo_weather st/2018 downloads/20180919/Endalen_Weatherstn_SSE.xlsx", skip = 1) %>%
  select(-`#`, -`Time, GMT+00:00`) %>%
  mutate(file = "raw_data/climate/DATA_ITEX_2015_2018/Hobo_weather st/2018 downloads/20180919/Endalen_Weatherstn_SSE.xlsx") %>%
  rename(
    DateTime = Date,
...
WeatherStation <- list.files(path = "raw_data/climate/DATA_ITEX_2015_2018/Hobo_weather st/", recursive = TRUE, full.names = TRUE, pattern = "csv$|\\.xls") %>%
